...
def ensure_config_file_exists(
    ...
    try:
        ...
        with open(file_path, "wt", encoding="utf-8") as output_file:
            with urllib.request.urlopen(config_url) as input_file:
                output_file.write(input_file.read().decode("utf-8"))
    except BaseException as err:
        LOG.error(
            "Could not read default config file %r from %r! %s",
            file_path,
            config_url,
            err,
        )
...
def check_requirements(install_dir: str = os.curdir) -> None:
    ...
    try:
        ...
        if docker_major < 20 or (docker_major == 20 and docker_minor < 10):
            message = (
        ...
        LOG.error(
            ...
            "runnable by the current user: %s",
            err,
        )
        raise
    ...
    try:
        ...
        LOG.error("The docker service does not appear to be running: %s", err)
        raise
...
def update_config(
    ...
    try:
        ...
        with open(config_file, "rt", encoding="utf=8") as file_handle:
        ...
        with open(config_file, "wt", encoding="utf-8") as file_handle:
            file_handle.write(text)
    except IOError as err:
        LOG.error("Error updating config file %r: %s", config_file, err)
        raise
...
def open_browser_after_startup() -> None:
    ...
    while not started:
        try:
            ...
            LOG.error("Skipping opening web browser tab to: http://localhost")
            raise
        started = (
...
def main(raw_args: list[str] = sys.argv[1:]) -> int:
    ...
    try:
        ...
        if args.command not in _DOCKER_CMDS:
            if args.command == "setup":
                ...
                remove(args.install_dir)
            elif args.command == "remove-all":
                remove(args.install_dir, remove_images=True)
            elif args.command == "update":
                update_images(args.install_dir)
            return 0

        # Process any docker wrapper command.
    ...
    LOG.critical(
        "The parser let through an unknown command somehow: %r", args.command
    )
    return 1
